...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.getElementById('datetime-scatter-chart')) {
        ...
        const datetimeScatterOptions = {
            series: [{
                name: 'Sales',
                data: generateDayWiseTimeSeries(new Date('01 Jan 2020'), 30, { min: 10, max: 90 })
            }, {
                name: 'Web Traffic',
                data: generateDayWiseTimeSeries(new Date('01 Jan 2020'), 30, { min: 50, max: 150 })
            }],
            chart: {
                height: 350,
                type: 'scatter',
        ...
        const imagesScatterOptions = {
            ...
            tooltip: {
                custom: function({ series, seriesIndex, dataPointIndex, w }) {
                    const platforms = ['Messenger', 'Instagram', 'Twitter', 'LinkedIn'];
    ...
    // // Add a theme changing function as an example for dynamic updates
    // function updateChartTheme() {
    //     // This function can be called when the theme changes
    //     if (window.ApexCharts) {
    //         window.ApexCharts.exec('basic-scatter-chart', 'updateOptions', {
    //             colors: ['var(--primary-500)', 'var(--danger-500)']
    //         }, false, true);
            
    //         // You would repeat this for all charts that need updating
    //     }
    // }

    // // Example: Attach to a theme toggle button if it exists
