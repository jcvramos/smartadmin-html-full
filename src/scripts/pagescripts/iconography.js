...
const styleSheets = {
    'sa': null, //included by default
    'base': null, //included by default
    'svg': null, //included by default
    'fal': './css/fa-light.css',
    'fas': './css/fa-solid.css',
    'far': './css/fa-regular.css',
    'fab': './css/fa-brands.css',
    'fad': './css/duotone.css',
    'material': './css/material-icons.css'
};

function updateStylesheet(iconSet) {
    const link = document.getElementById('dynamic-style') || document.createElement('link');
    if (!link.id) {
        link.id = 'dynamic-style';
        link.rel = 'stylesheet';
        document.head.appendChild(link);
    }
    if (styleSheets[iconSet]) {
        link.href = styleSheets[iconSet];
        link.onload = () => console.log(`${iconSet} stylesheet loaded`);
        link.onerror = () => console.error(`Failed to load ${iconSet} stylesheet`);
    } else {
...
function generateIconList(icons, iconPrefix) {
    ...
    iconList.innerHTML = icons.map(icon => {
        ...
        return `
            <li class="col-4 col-sm-3 col-md-3 col-lg-2 col-xl-2 col-xxl-2 col-xxxl-1 d-flex justify-content-center align-items-center mb-g" data-icon-name="${displayName}">
...
const getIconClass = (prefix, icon) => {
    ...
    switch (prefix) {
        case 'svg':
            ...
            return `<svg class="sa-icon ${weightClass}${fillClass}"><use href="img/sprite.svg#${cleanIcon}"></use></svg>`;
        case 'fal':
